export default (element, options) => {
  document
    .querySelectorAll(".vh-paopao")
    .forEach((item) => setTimeout(() => item.remove()));
  const config = Object.assign(
    { radius: 10, density: 0.3, clearOffset: 0.2 },
    options
  );
  let width,
    height,
  ...
  class Particle {
    ...
    reset() {
      ...
      this.color =
        config.color === "random"
          ? `rgba(${(Math.random() * 255) | 0},0,0,${Math.random().toFixed(2)})`
          : config.color;
    }
    draw() {
  ...
  Array.from({ length: (width * config.density) | 0 }, () =>
