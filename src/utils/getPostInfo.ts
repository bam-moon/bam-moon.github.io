...
const getCategories = () => {
  ...
  return Object.entries(categoriesList).map(([title, count]) => ({
    title,
    count,
  }));
};
...
const getRecommendArticles = () => {
  ...
  return (recommendList.length ? recommendList : posts.slice(0, 6)).map(
    (i) => ({ title: i.data.title, date: i.data.date, id: i.data.id })
  );
};
