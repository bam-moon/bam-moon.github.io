...
const renderSearch = (arr: any[]) => {
  searchHTML = !arr.length
    ? "<em></em>"
    : arr
        .map(
          (i) =>
            `<a class="vh-search-item" href="${i.url}"><span class="vh-ellipsis">${i.title}</span><p class="vh-ellipsis line-3">${i.content}</p></a>`
        )
        .join("");
  document.querySelector(
    ".vh-header>.main>.vh-search>main>.vh-search-list"
...
// 初始化搜索框
const vhSearchInit = () => {
  const searchDOM: any = document.querySelector(
    ".vh-header>.main>nav>span.search-btn"
  );
  const searchMainDOM: any = document.querySelector(
    ".vh-header>.main>.vh-search>main"
  );
  const searchListDOM: any = document.querySelector(
    ".vh-header>.main>.vh-search"
  );
  const addActive = () =>
    setTimeout(() => {
  ...
  const removeActive = () =>
    setTimeout(() => searchListDOM.classList.remove("active"));
  // 禁止默认事件
  ...
  searchListDOM
    ...
    .addEventListener("input", searchInputChange);
};
