...
const TwikooFn = async (commentDOM: string) => {
  ...
  twikoo.init({
    ...
    onCommentLoaded: () =>
      setTimeout(() =>
        document
          .querySelectorAll('.vh-comment a[href="#"]')
...
const WalineFn = async (commentDOM: string, walineInit: any) => {
  ...
  walineInit = init({
    ...
    emoji: [
      "https://registry.npmmirror.com/@waline/emojis/1.3.0/files/alus",
    ...
    imageUploader: async (file: any) => {
      ...
      const res = await fetch("https://wp-cdn.4ce.cn/upload", {
        method: "POST",
        body,
      });
...
const checkComment = () => {
  ...
  const CommentItem = CommentARR.find(
    (i: keyof typeof SITE_INFO.Comment) => SITE_INFO.Comment[i].enable
  );
  return CommentItem;
};
